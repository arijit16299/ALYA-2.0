		...
		switch (args[0]) {
			...
			case "+": {
				...
				return message.reply(
					(notWLTIDs.length > 0 ? getLang("added", notWLTIDs.length,
						getNames.filter(({ tid }) => notWLTIDs.includes(tid))
							.map(({ tid, name }) => `├‣ 𝚃𝙷𝚁𝙴𝙰𝙳 𝙽𝙰𝙼𝙴: ${name}\n╰‣ 𝚃𝙷𝚁𝙴𝙰𝙳 𝙸𝙳: ${tid}`).join("\n")
					) : "")
					+ (threadIDs.length > 0 ? getLang("alreadyWLT", threadIDs.length,
						threadIDs.map(tid => `╰‣ 𝚃𝙷𝚁𝙴𝙰𝙳 𝙸𝙳: ${tid}`).join("\n")
					) : "")
				);
			}
			...
			case "-": {
				...
				return message.reply(
					(threadIDs.length > 0 ? getLang("removed", threadIDs.length,
						getNames.map(({ tid, name }) => `├‣ 𝚃𝙷𝚁𝙴𝙰𝙳 𝙽𝙰𝙼𝙴: ${name}\n╰‣ 𝚃𝙷𝚁𝙴𝙰𝙳 𝙸𝙳: ${tid}`).join("\n")
					) : "")
					+ (notWLTIDs.length > 0 ? getLang("notAdded", notWLTIDs.length,
						notWLTIDs.map(tid => `╰‣ 𝚃𝙷𝚁𝙴𝙰𝙳 𝙸𝙳: ${tid}`).join("\n")
					) : "")
				);
			}
			...
			case "-l": {
				const getNames = await Promise.all(
					config.whiteListModeThread.whiteListThreadIds.map(async tid => {
				...
				return message.reply(
					getLang("listWLTs", getNames.map(({ tid, name }) =>
						`├‣ 𝚃𝙷𝚁𝙴𝙰𝙳 𝙽𝙰𝙼𝙴: ${name}\n├‣ 𝚃𝙷𝚁𝙴𝙰𝙳 𝙸𝙳: ${tid}`
					).join("\n"))
				);
			}
			...
			case "-m": {
				if (args[1] === "on") {
					config.whiteListModeThread.status = true;
					writeFileSync(global.client.dirConfig, JSON.stringify(config, null, 2));
					return message.reply(getLang("turnedOn"));
				}
				else if (args[1] === "off") {
					config.whiteListModeThread.status = false;
					writeFileSync(global.client.dirConfig, JSON.stringify(config, null, 2));
					return message.reply(getLang("turnedOff"));
				}
				else if (args[1] === "noti" && args[2] === "on") {
					config.whiteListModeThread.noti = true;
					writeFileSync(global.client.dirConfig, JSON.stringify(config, null, 2));
					return message.reply(getLang("turnedOnNoti"));
				}
				else if (args[1] === "noti" && args[2] === "off") {
